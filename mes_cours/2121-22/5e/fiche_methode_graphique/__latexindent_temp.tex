...
\begin{titlebox}{\titre}
    ...
    \vspace{-7pt}
    \notebox{
            On est pas obligé de commencer par 0°C en ordonnée!
            La température ambiante de l'eau étant d'environ 25°C
            On pourra commencer par 20°C.
         }
    \end{minipage}
    \hspace{20pt}
    \begin{minipage}[c]{0.5\textwidth}
        ...
        \begin{center}        
                \begin{tikzpicture}[scale=0.5]
                    % \draw[color=gray] (-1,-1) grid (10,10);
                    \draw[very thin,color=gray,step=1] (0,0) grid (10,10);
                    \draw[->,very thick] (0,0) -- (0,10) node[below right] {Température en °C};
                    \draw[->,very thick] (0,0) -- (10,0);
                    \node[above right] at (7,0) {Temps en secondes};
                    \foreach \x in {0,...,9} {
                        % \let\xx=\x/2+1
                        \draw[thick] (\x,0) -- (\x,-0.2) node[below left] {
                            \tiny \directlua{tex.sprint((\x*10))}
                            };
                            \draw[thick] (0,\x) -- (-0.2,\x) node[left]{
                                \tiny \directlua{tex.sprint((\x*5))}
                            };
                    }
                \end{tikzpicture}
        \end{center}
